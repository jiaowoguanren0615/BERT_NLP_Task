...
from util.optimizer import yield_optimizer
from util.engine import train_one_epoch, evaluate
from util.losses import loss_function
from util.lr_sched import adjust_learning_rate, create_lr_scheduler

from datesets import build_dataset

from models import build_model


def get_args_parser():
    parser = argparse.ArgumentParser('BERT training and evaluation script', add_help=False)
    parser.add_argument('--batch-size', default=8, type=int)
    parser.add_argument('--epochs', default=5, type=int)

    # Model parameters
    parser.add_argument('--bert_path', type=str,
                        default='/usr/local/MyObjData/jupyterCode/CommonLit-Readability-Prize/bert-large-uncased')
    parser.add_argument('--model', default='BERT_LARGE_UNCASED', type=str, metavar='MODEL',
                        choices=['BERT_BASE_UNCASED', 'BERT_LARGE_UNCASED', 'DBERT_BASE_UNCASED'],
                        help='Name of model to train')
    parser.add_argument('--model-ema', action='store_true')
    parser.add_argument('--max_len', type=int, default=185,)
    ...
    # Dataset parameters
    parser.add_argument('--data_root', default='/usr/local/MyObjData/jupyterCode/CommonLit-Readability-Prize/train.csv', type=str,
                        help='dataset path')
...
def main(args):
    ...
    optimizer = yield_optimizer(model_without_ddp, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(
            f"Loss of the network on the {len(dataset_val)} test text-data: {test_stats['valid_loss']:.4f}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('BERT training and evaluation script', parents=[get_args_parser()])
