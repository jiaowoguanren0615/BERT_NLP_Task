...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.MSELoss,
    ...
    for idx, inputs in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            outputs = model(ids=ids, mask=mask, token_type_ids=ttis).view(-1)
